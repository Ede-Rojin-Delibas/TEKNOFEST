...
from netHesapla import net_hesapla
from tahmin_et import tahmin_yap
...
def tahmin_et():
    ...
    # 4. Net Hesaplama
    try:
        net_sonuc = net_hesapla(sinav_turu, dogru_yanlis_verileri)
    except Exception as e:
        return jsonify({"error": f"Net hesaplama hatası: {str(e)}"}), 500

    # 5. Sıralama Tahmini
    try:
        tahmini_siralama = tahmin_yap(sinav_turu, net_sonuc)  # net_sonuc artık direkt float değer
    except Exception as e:
    ...
    response = {
        ...
        "net": net_sonuc,  # Artık tek bir değer
        "siralama": {
